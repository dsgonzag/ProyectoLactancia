...
import com.packages.prolactancia.Controladores.clsDatabase;
import com.packages.prolactancia.Controladores.clsGeolocalizacion;
        ...
        try {
            ...
            if (liDistanciasCalculadas.size() > 0) {
                ...
                if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    poDistanciaMinima = liDistanciasCalculadas.stream().min(new Comparator<clsGeolocalizacion>() {
                        @Override
                        public int compare(clsGeolocalizacion first, clsGeolocalizacion second) {
                            return Double.compare(first.getLiDistancia(), second.getLiDistancia());
                        }
                    }).get();
                }
            }
            ...
            Toast.makeText(Maps.this, "Error al momento de calcular distancias en el metodo lCalcularDistancia()", Toast.LENGTH_LONG).show();
        }
